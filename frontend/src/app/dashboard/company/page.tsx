...
import { Users, FileText, MessageSquare, Bell, Plus, CheckCircle, Clock, Building2, TrendingUp } from "lucide-react"
...
export default function CompanyDashboard() {
  ...
  const recentActivity = [
    ...
    { id: 2, type: "report", message: "Weekly report submitted by John Smith", time: "3 hours ago" },
    { id: 3, type: "intern", message: "Alice Brown started her internship today", time: "1 day ago" },
    { id: 4, type: "project", message: "E-commerce Platform project milestone completed", time: "2 days ago" },
  ]
  ...
  const topInterns = [
    { id: 1, name: "John Smith", position: "Software Developer", progress: 85, rating: 4.8 },
    { id: 2, name: "Alice Brown", position: "Data Analyst", progress: 78, rating: 4.6 },
    { id: 3, name: "Mike Johnson", position: "UI/UX Designer", progress: 92, rating: 4.9 },
  ]
    ...
    <DashboardLayout requiredRole="company">
      <div className="space-y-8 bg-gray-50 min-h-screen p-6">
        ...
        <div>
          ...
          <p className="text-gray-600">Welcome back! Here's your internship program overview.</p>
        </div>
            ...
            <CardContent>
              <div className="space-y-4">
                {upcomingTasks.map((task) => (
                  <div key={task.id} className="flex items-center justify-between p-3 rounded-lg border border-gray-200 bg-gray-50">
                    <div>
                      <p className="font-medium">{task.task}</p>
                      <p className="text-sm text-gray-600">{task.count} item(s)</p>
                    </div>
                    <Badge className={getPriorityColor(task.priority) + " px-3 py-1 rounded-full text-xs font-semibold"}>
                      {task.priority}
                    </Badge>
                  </div>
                ))}
        ...
        <Card className="border-2 border-gray-200 shadow-sm rounded-xl bg-white">
          <CardHeader>
            ...
            <CardDescription>Your highest-rated interns this month</CardDescription>
          </CardHeader>
          <CardContent>
            <div className="space-y-4">
              {topInterns.map((intern) => (
                <div key={intern.id} className="flex items-center justify-between p-4 rounded-lg border border-gray-200 bg-gray-50">
                  ...
                  <div className="flex items-center space-x-4">
                    <div className="text-right">
                      <p className="text-sm font-medium">{intern.progress}% Complete</p>
                      <p className="text-sm text-gray-600">⭐ {intern.rating}/5.0</p>
                    </div>
                    <div className="w-20 bg-gray-200 rounded-full h-2">
                      <div className="bg-green-600 h-2 rounded-full" style={{ width: `${intern.progress}%` }}></div>
                    </div>
                  </div>
                </div>
              ))}
